create table T_ARTICLE ( ART_ID_C varchar(36) not null, ART_IDFEED_C varchar(36) not null, ART_URL_C varchar(2000), ART_GUID_C varchar(2000) not null, ART_TITLE_C varchar(4000), ART_CREATOR_C varchar(200), ART_DESCRIPTION_C LONGVARCHAR, ART_COMMENTURL_C varchar(2000), ART_COMMENTCOUNT_N int, ART_ENCLOSUREURL_C varchar(2000), ART_ENCLOSURELENGTH_N int, ART_ENCLOSURETYPE_C varchar(2000), ART_PUBLICATIONDATE_D datetime not null, ART_CREATEDATE_D datetime not null, ART_DELETEDATE_D datetime, primary key (ART_ID_C) );
create index IDX_ART_GUID_C on T_ARTICLE ( ART_GUID_C );
create table T_AUTHENTICATION_TOKEN ( AUT_ID_C varchar(36) not null, AUT_IDUSER_C varchar(36) not null, AUT_LONGLASTED_B bit not null, AUT_CREATIONDATE_D datetime not null, AUT_LASTCONNECTIONDATE_D datetime, primary key (AUT_ID_C) );
create table T_BASE_FUNCTION ( BAF_ID_C varchar(20) not null, primary key (BAF_ID_C) );
create table T_CATEGORY ( CAT_ID_C varchar(36) not null, CAT_IDUSER_C varchar(36) not null, CAT_IDPARENT_C varchar(36), CAT_NAME_C varchar(100), CAT_ORDER_N int not null, CAT_FOLDED_B bit not null, CAT_CREATEDATE_D datetime not null, CAT_DELETEDATE_D datetime, primary key (CAT_ID_C) );
create table T_CONFIG ( CFG_ID_C varchar(50) not null, CFG_VALUE_C varchar(250) not null, primary key (CFG_ID_C) );
create table T_FEED ( FED_ID_C varchar(36) not null, FED_RSSURL_C varchar(2000) not null, FED_URL_C varchar(2000), FED_TITLE_C varchar(100), FED_LANGUAGE_C varchar(10), FED_DESCRIPTION_C varchar(4000), FED_CREATEDATE_D datetime not null, FED_LASTFETCHDATE_D datetime, FED_DELETEDATE_D datetime, primary key (FED_ID_C) );
create table T_FEED_SUBSCRIPTION ( FES_ID_C varchar(36) not null, FES_IDUSER_C varchar(36) not null, FES_IDFEED_C varchar(36) not null, FES_IDCATEGORY_C varchar(36) not null, FES_TITLE_C varchar(100), FES_ORDER_N int not null, FES_CREATEDATE_D datetime not null, FES_DELETEDATE_D datetime, primary key (FES_ID_C) );
create table T_LOCALE ( LOC_ID_C varchar(10) not null, primary key (LOC_ID_C) );
create table T_USER ( USE_ID_C varchar(36) not null, USE_IDLOCALE_C varchar(10) not null, USE_USERNAME_C varchar(50) not null, USE_PASSWORD_C varchar(60) not null, USE_EMAIL_C varchar(100) not null, USE_THEME_C varchar(100) not null, USE_DISPLAYTITLEWEB_B bit not null, USE_DISPLAYTITLEMOBILE_B bit not null, USE_DISPLAYUNREADWEB_B bit not null, USE_DISPLAYUNREADMOBILE_B bit not null, USE_FIRSTCONNECTION_B bit not null, USE_CREATEDATE_D datetime not null, USE_DELETEDATE_D datetime, primary key (USE_ID_C) );
create table T_USER_ARTICLE ( USA_ID_C varchar(36) not null, USA_IDUSER_C varchar(36) not null, USA_IDARTICLE_C varchar(36) not null, USA_CREATEDATE_D datetime not null, USA_READDATE_D datetime, USA_STARREDDATE_D datetime, USA_DELETEDATE_D datetime, primary key (USA_ID_C) );
create table T_USER_BASE_FUNCTION ( UBF_ID_C varchar(36) not null, UBF_IDUSER_C varchar(36) not null, UBF_IDBASEFUNCTION_C varchar(20) not null, UBF_CREATEDATE_D datetime not null, UBF_DELETEDATE_D datetime, primary key (UBF_ID_C) );
...
alter table T_USER_BASE_FUNCTION add constraint FK_UBF_IDBASEFUNCTION_C foreign key (UBF_IDBASEFUNCTION_C) references T_BASE_FUNCTION (BAF_ID_C) on delete restrict on update restrict;
alter table T_USER_BASE_FUNCTION add constraint FK_UBF_IDUSER_C foreign key (UBF_IDUSER_C) references T_USER (USE_ID_C) on delete restrict on update restrict;
